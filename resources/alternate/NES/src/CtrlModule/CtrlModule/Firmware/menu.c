...
int Menu_Run()
{
	...
	if((TestKey(KEY_ESC)&2) || (( joy_pins & 0x40 ) > 0 )) //drive button
	{
		while(TestKey(KEY_ESC))
			HandlePS2RawCodes(); // Wait for KeyUp message before opening OSD, since this disables the keyboard for the  core.
		OSD_Show(menu_visible^=1);
	}
